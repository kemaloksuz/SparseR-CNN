...
class FastaLRPLoss(torch.autograd.Function):
    @staticmethod
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5): 
        classification_grads=torch.zeros(logits.shape).cuda()
        ...
        #x_ij s as score differences with bgs
        bg_relations = relevant_bg_logits[:,None]-fg_logits[None,:]
        ...
        #Compute the rank of the example within fgs and number of bgs with larger scores
        rank_pos = 1+fg_relations.sum(axis=0)
        FP_num = bg_relations.sum(axis=0)
        ...
        classification_grads[fg_labels] = -(torch.sum(fg_relations*regression_losses, axis=0)+FP_num)/rank
        classification_grads[relevant_bg_labels] = torch.sum((bg_relations*(-classification_grads[fg_labels] / FP_num)), axis=1)
