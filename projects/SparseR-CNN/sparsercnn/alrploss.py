...
class SetaLRPLossCriterion(nn.Module):
...
#        self.aLRP_Loss = APLoss()
    ...
    def forward(self, outputs, targets):
        ...
        idx = self._get_src_permutation_idx(indices)
        src_boxes.append(outputs['pred_boxes'][idx])
        ...
        target_classes_[idx] = target_classes_o
        ...
        if labels.sum() > 0:
            ...
            if class_loss < self.loss_weight*0.99:
                ...
                losses['loss_giou'] = 2*(losses_giou * self.giou_SB_weight)
                losses['loss_bbox'] = 2*((losses_bbox * self.bbox_SB_weight) / 2.5)
            else:
                losses['loss_giou'] = 2*losses_giou
                losses['loss_bbox'] = 2*losses_bbox
        else:
            losses = {}
